...
function ScreenMyArticles(props) {
  ...
  console.log("Le composent ScreenMyArticle le recoit");
     ...
     var wishCards=  props.likeToDisplay.map((article, i)=>{
    return(
      <Card
        style={{  
          width: 300, 
        ...
        actions={[
          <Link to="/article-source"><Icon type="read" key="ellipsis2" /></Link>
          ,
            <Icon type="delete" onClick={ ()=> props.deleteToWishList(article) } key="ellipsis" />
        ]}
        >
  ...
  return (
    <div>        
            ...
            <div className="Banner"/>
              ...
              <div  style={{display:'flex',justifyContent:'center',backgroundColor:'yellow', maxHeight:'100%'}}>
                {wishCards}
              </div>
...
function mapStateToProps(state) {
  return { likeToDisplay: state.wishList }
  
}
function mapDeleteToProps(dispatch) {
  return {
    deleteToWishList: function(article) { 
        dispatch( {type: 'delete', articleToDelete: article} ) 
    }
  }
}
